...
export const CONFIG = {
  green: {
    ...
    shade: {
      ...
      /**
       * 濃淡がフルレンジで表す勾配（無次元、0.08 で 8%）。**読みの単位を決める値。**
       * シードによらず「明るさがこれだけ変わる ＝ 勾配が何 %」を一定にする。
       * 初期値はスティンプ 10ft の臨界勾配（7.8%＝ここを超える下りでは止まらない）に合わせてある
       */
      gradientRange: 0.08,
      /** 光の方位 [度]。太陽に相当する固定方向。プレイヤーが回っても地形の見え方が変わらないように固定 */
      lightAzimuthDeg: 35,
      /** 高さの濃淡の強さ。地形のマクロな高低が消えないよう弱く重ねる */
      heightStrength: 0.25,
      /** 高さの濃淡がフルレンジで表す高低差 [m] */
      heightRange: 0.5,
    },
  ...
  light: {
    /**
     * 勾配ベースの濃淡にしたぶん平均が暗くなったので、その埋め合わせで上げてある。
  ...
  game: {
    ...
    address: {
       ...
       * 右打ちはボールの右横に立ち、前傾したまま首を左（目標）へ向けるので頭は右へ傾く。
       * 頭を右へ傾けると網膜像は逆に回るので、鉛直な旗竿は画面上で上端が左へ倒れて見える。
       * その向きになる符号がマイナス
       */
      rollDeg: -30,
      /** READ からの遷移時間 [s] */
