...
export const CONFIG = {
  green: {
    ...
    /**
     * 濃淡を「高さそのもの」で出す案（比較用）。明るい ＝ 高い。
     * これも絶対スケールで、ハイトマップの 0 は必ずグリーン中央（全体傾斜もうねりも 0）なので、
     * 「同じ明るさ ＝ 同じ高さ」がシードによらず成り立つ。
     *
     * 高さでは読めないと判断した過去の計測（ライン上 1.6cm ＝ 明度差 3%）は、
     * レンジが 0.5m と視野の実態（半径6mで 20〜26cm）の倍あり、かつ振れ幅が 0.12 しかない
     * 設定でのもの。視野に合わせて 0.25m まで詰め、振れ幅を勾配版と同じ 0.75 に上げると、
     * 4.5m のライン上の高低差 2〜7cm が明度差 12〜41% になる
     */
    shadeHeight: {
      gradientStrength: 0,
      gradientRange: 0.03,
      ...
      /** 高さの濃淡の振れ幅（±） */
      heightStrength: 0.75,
      /** フルレンジで表す高低差 [m]。読みの視野（半径6m）の高低差に合わせる */
      heightRange: 0.25,
      heightPeriod: 0,
      /**
       * 繰り返し版で使う周期 [m]。使える色数は最終画像の量子化で 8 段ほどに頭打ちになるので、
       * それをレンジ全体（25cm ＝ 1段 3cm）に配るか、この周期に配り直すかの違い。
       * 4cm なら 1段 0.5cm まで細かくなり、レンジ外の飽和もなくなる
       */
      repeatPeriod: 0.04,
      levels: 0,
    },
  ...
  pixel: {
    ...
    levels: 8,
    /**
     * 高さの濃淡のときの段数。1段が何 cm かは heightRange /(2 × 振れ幅 × 段数) で決まり、
     * 0.25m / (2 × 0.75 × 16) ＝ 約 1cm。等高線の間隔がそのまま読みの目盛りになる
     */
    heightLevels: 16,
    /**
