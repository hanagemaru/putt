...
export const CONFIG = {
  ...
  pixel: {
     ...
     * heightRange /(2 × 振れ幅 × 段数) で決まり、0.25 /(2 × 0.75 × 16) ＝ 約 1cm。
     * 等高線の間隔がそのまま読みの目盛りになる。
     ...
     *
     * ただし絞りすぎると濃淡の段まで潰れる。実測（READ ボール後方の芝の色数）：
     * 12 で 6〜9 色、20 で 9〜10 色、32 で 11〜12 色、連続なら 25〜26 色。
     * 計測上は 20 付近が等高線を強く感じやすかったが、実機での見た目を優先して 32 を正式採用。
     * 高さの読みの単位は levels=16（約 1cm/段）で維持し、colorLevels は表示上の量子化だけを担う。
     */
    colorLevels: 32,
  },
