 ...
 * 長い音源ファイルを持たず、PSG風の矩形波・三角波・ノイズだけで鳴らす。
 */
...
const LOOK_AHEAD_SEC = 24;
const SCHEDULER_MS = 4000;
...
export class PuttMusic {
  ...
  /**
   * ホールアウトは勝利ファンファーレにせず、BGMと同じ音色の短い C6/9 スタブで締める。
   * カップ落下音を先に聞かせるため、呼び出し側から少し遅延させる。
   */
  playHoleOutJingle(delay = 0.32): void {
    ...
    // ジングルの間だけBGMを少し引かせ、実録のカップ音とぶつけない。
  ...
  private scheduleCycle(scene: MusicScene, start: number, bus: GainNode): void {
    ...
    for (let b = 0; b < settings.bars; b++) {
      ...
      const chordHits = scene === 'play' ? [0, 2.5] : [0, 1.5, 2.75];

      // ベタ押しせず、6/9・sus・9th・13thを短いスタブで置く。
      ...
      // 試聴で選んだ、低すぎない三角波ベース。ルートだけでなく5度・経過音も動かす。
      const bassline =
        scene === 'play'
          ? [
              [0, chord.root, 0.052],
    ...
    if (scene === 'menu') {
      ...
      for (let k = 0; k < MOTIF.length; k++) {
        this.scheduleSquare(MOTIF[k] + 12, base + k * beat * 0.5, beat * 0.22, 0.009, bus, 3400);
      }
    } else if (scene === 'play') {
      for (const baseBars of [1.2, 4]) {
        const base = start + baseBars * bar;
        for (let k = 0; k < 4; k++) {
          this.scheduleSquare(MOTIF[k] + 12, base + k * beat * 0.62, beat * 0.2, 0.004, bus, 3200);
        }
      }
    } else {
      for (let k = 0; k < MOTIF.length; k++) {
        this.scheduleSquare(MOTIF[k] + 12, start + k * beat * 0.5, beat * 0.22, 0.01, bus, 3400);
      }
